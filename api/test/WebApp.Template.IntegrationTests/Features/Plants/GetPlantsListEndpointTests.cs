...
public class GetPlantsListEndpointTests(IntegrationTestFixture fixture, ITestOutputHelper outputHelper)
    : TestClass<IntegrationTestFixture>(fixture, outputHelper)
{
    ...
    [InlineData(1, 5, "plantId", "asc", "Pla")]
    [InlineData(1, 5, "plantId", "desc", "1")]
    [InlineData(1, 5, "status", "asc", "")]
    [InlineData(1, 5, "status", "desc", "5")]
    [InlineData(1, 5, "capacityDc", "asc", "")]
    [InlineData(1, 5, "capacityDc", "desc", "3")]
    public async Task Should_Return_Plants(int page, int pageSize, string sortBy, string sortDirection, string search)
    {
        ...
        result.Result!.Count().Should().Be(expectedPlants.Length);
